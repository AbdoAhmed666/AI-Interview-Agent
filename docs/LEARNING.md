...
```
components/interview/AnswerEditor.tsx
  → hooks/useInterview.ts          submit()
  → lib/axios.ts                   the request leaves the browser
...
`backend/test_adaptive_interview_route.py` · `backend/test_rate_limit.py` ·
...
- What is a fixture? What does `monkeypatch` actually replace, and when is it
  the wrong tool?
- Read `test_rate_limit.py::test_a_rejected_request_is_not_counted`. What
  behaviour would be wrong if that test did not exist?
...
**Files:** `Dockerfile` · `frontend/Dockerfile` · `docker-compose.yml` ·
`backend/docker-entrypoint.sh` · `.github/workflows/ci.yml` · `DEPLOY.md`
...
- Why must `NEXT_PUBLIC_API_URL` be known at build time, when `DATABASE_URL`
  need not be?
- `rate_limit.py`: why two limits? Which one actually caps the bill, and why
  can the other be walked around?
...
| 7 | Two rate limits instead of one |
| 8 | JWT in localStorage — and why it should change |
| 9 | Hugging Face Spaces for the backend |
